...
import imageio.v2 # Newer than 2.2.0 must be used to use pilmode e.g. pip install git+https://github.com/imageio/imageio.git
...
def plot_result_images(images):
    ...
    def plot_result_image(image, lines_unfiltered, lines_filtered):
        ...
        axes[0][0].imshow(imageio.v2.imread(file + ending, pilmode='L'), cmap='gist_gray')
        axes[0][0].set_title('Image to be analyzed')
        axes[0][0].axis('off')

        axes[0][1].imshow(imageio.v2.imread(file + '_wo_bkgnd' + ending, pilmode='L'), cmap='gist_gray')
        axes[0][1].set_title('Removed backround')
        axes[0][1].axis('off')

        axes[1][0].imshow(imageio.v2.imread(file + '_unfiltered_lines' + ending, pilmode='L'), cmap='gist_gray')
        axes[1][0].set_title('Detected ' + lines_unfiltered + ' lines before filtering')
        axes[1][0].axis('off')

        axes[1][1].imshow(imageio.v2.imread(file + '_filtered_lines' + ending, pilmode='L'), cmap='gist_gray')
...
def align_images(images):
    ...
    def align_image(path_analyse, img_background):
        # Load analyse image
        img_analyse = imageio.v2.imread(path_analyse, pilmode='L')
    ...
    # Start here
    img_background = imageio.v2.imread(os.path.join('background', 'background.jpg'), pilmode='L')
    ...
    for path_analyse in images:
        ...
        imageio.v2.imsave(file + '_align' + ending, img_aligned, format='jpg')
...
def remove_backgrounds(images):
    ...
    def remove_background(path_analyse, img_background):
        img_analyse = imageio.v2.imread(path_analyse, format='JPEG-PIL', pilmode='L')
    ...
    img_background = imageio.v2.imread(os.path.join('background', 'background_with_stripes.jpg'), pilmode='L')
    # ‘L’ (8-bit pixels, black and white)
    ...
    for path_analyse in images:
        ...
        file, ending = os.path.splitext(path_analyse)
        imageio.v2.imsave(file[:-6] + '_wo_bkgnd' + ending, img_analyse_wo_bkgnd, format='jpg')
...
def filter_lines(images):
    ...
    for filename in filenames:
        ...
        img_analyse = imageio.v2.imread(file + '_wo_bkgnd' + ending, pilmode='L')
        img_analyse_lines = util.colorImageWithLines(lines_filtered, img_analyse)

        imageio.v2.imsave(file + '_filtered_lines' + ending, img_analyse_lines, format='jpg')
